...
func findBundleHeaderOffset(r io.ReadSeeker, sections []pe.SectionHeader32) (int64, error) {
	// PointerToRawData and SizeOfRawData are user-controlled uint32s whose sum can span well past the
	// real file. That only scopes how far FindSignatureOffset scans; nothing is allocated from it.
	return bundle.FindSignatureOffset(r, calculatePEEndOffset(sections))
}
